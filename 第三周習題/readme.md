# 數學原理說明書
對話連結：https://chatgpt.com/share/695713f5-9ffc-8009-b3bf-b9ff444acc96
## 1️⃣ 直線與圓交點的代數意義
...
- 方法二：斜率垂直（Slope）  
  - 若直線斜率 $m_1$，垂線斜率 $m_2$ 滿足：  
    $m_1 \cdot m_2 = -1$
- 運算公式：
  - 若直線為 $ax + by + c = 0$，點為 $P(x_0, y_0)$，則垂足 $H(x_h, y_h)$ 為：
    $$
    x_h = x_0 - a \frac{a x_0 + b y_0 + c}{a^2 + b^2}, \quad
    y_h = y_0 - b \frac{a x_0 + b y_0 + c}{a^2 + b^2}
    $$
...
- 相對於中心點 $C(x_c, y_c)$：
  $$
  (x, y) \to (x', y') = (x_c + s(x - x_c), \; y_c + s(y - y_c))
  $$
- 若以原點為中心：
  $$
  (x, y) \to (s x, \; s y)
